...
def process_command(command: str, data: list[int],
    ...
    """Processa una comanda delete o number, modificant el contingut de la cua
    a data, i retornant els nous valors per el minim, maxim, la suma i el
    nombre d'elements de la cua.
    """
    if command == "delete":
        if len(data) != 0:
            ultim = data[0]
